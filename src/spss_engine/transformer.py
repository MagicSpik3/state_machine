...
from spss_engine.parsers.data_loader import DataLoaderParser  # 🟢 NEW IMPORT

class CommandTransformer:
    def __init__(self):
        self.extractor = AssignmentExtractor()
        self.data_loader = DataLoaderParser()  # 🟢 NEW COMPONENT
    ...
    def _clean_filename(self, match) -> Optional[str]:
        ...
        if match.group(2): return match.group(2).rstrip('.')
        return None
    ...
    def transform(self, command: ParsedCommand) -> List[SemanticEvent]:
        ...
        if command.type == TokenType.FILE_READ:
            ...
            # This extracts delimiter, variables, header logic, etc.
            read_event = self.data_loader.parse(command.raw)
            
            # Destructive load logic
            ...
            for quoted, unquoted in matches:
                if quoted: f = quoted
                else: f = unquoted.rstrip('.')
                if f and f != "*": files.append(f)
