...
class TestScopeCorpus:
    ...
    def test_comprehensive_corpus_parsing(self):
        ...
        assert pipeline.get_variable_version("Gender") == "GENDER_1"

        # 3. Check Arithmetic & Whitespace
        # Income_2023 init(-1) -> assigned(math)
        assert pipeline.get_variable_version("Income_2023") == "INCOME_2023_1"

        # 4. Check Conditional Logic Extraction
        # 'Description' set in IF (DESCRIPTION_0) -> Recoded in DO IF (DESCRIPTION_1)
        assert pipeline.get_variable_version("Description") == "DESCRIPTION_1"
        ...
        assert pipeline.get_variable_version("Poverty_Flag") == "POVERTY_FLAG_0"